...
char* train_path = "/Users/Joey/Desktop/RForrest_train/history_forrest_train.txt";
char* test_path = "/Users/Joey/Desktop/RForrest_train/history_forrest_test.txt";
int datamode = 0;
int screenPoint = 0;
int train = 0;
...
#define NUMBER_OF_TRAINING_SAMPLES 1185
#define ATTRIBUTES_PER_SAMPLE 141
#define NUMBER_OF_TESTING_SAMPLES 136
...
for (int i=0; i<ModelLeftMatrix.rows; i++) {
 ...
 std::vector<std::vector<float> > row_history;
 int frame_counter = 0;
...
void render_face_detections (cv::Mat& image, cv::Rect& area, cv::Size& size, const std::vector<rectangle>& trackers, const std::vector<rectangle>& faces, const std::vector<full_object_detection>& dets, const cv::Scalar color)
{
  ...
  std::vector<float> allKeyPointsX, allKeyPointsY, row_train;
...
// Training Data Collection interface for Random Forrests
  if(LeftEllipse.size() == 6 && RightEllipse.size() == 6 && screenPoint == 1 && !row_history.empty()){
    circle(image,cv::Point(10 ,10), 5, cv::Scalar(0,255,0), 5);

    if(train == 0)
      trainForrest.open("/Users/Joey/Desktop/RForrest_train/history_forrest_train.txt", std::ios_base::app);
    else
      trainForrest.open("/Users/Joey/Desktop/RForrest_train/history_forrest_test.txt", std::ios_base::app);
        
    std::vector<std::vector<float> >::iterator it=row_history.begin(), end=row_history.end();
    while (it!=end) 
    {
        std::vector<float>::iterator it1=it->begin(),end1=it->end();
        std::copy(it1,end1,std::ostream_iterator<double>(trainForrest, ","));
        ++it;
    }
  ...
  if(LeftEllipse.size() == 6 && RightEllipse.size() == 6 && screenPoint == 2 && !row_history.empty()){
  ...
  if(LeftEllipse.size() == 6 && RightEllipse.size() == 6 && screenPoint == 3 && !row_history.empty()){
  ...
  if(LeftEllipse.size() == 6 && RightEllipse.size() == 6 && screenPoint == 4 && !row_history.empty()){
...
/********************** Unix Socket Initialization END ****************************/
  ...
  /***************** RANDOM TREES ****************/
            ...
            read_data_from_csv(test_path, testing_data, testing_classifications, NUMBER_OF_TESTING_SAMPLES))
    {
        ...
        rtree->save("history_Random_forrest_classifier.yml");
      ...
      rtree->load("history_Random_forrest_classifier.yml");
    }
        while(1)
        {
