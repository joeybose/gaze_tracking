...
int useForrest = 0; //Use Random Forrest
...
for (int i=0; i<ModelLeftMatrix.rows; i++) {
 ...
 std::vector<double> gazeStabilizer; //A Votring scheme to help reduce the jumpiness of the gaze point
...
/*This function does the following :
  ...
  6. Final Gaze Location is based on a voting scheme which uses past 7 frames to stabilize gaze
*/
...
void render_face_detections (cv::Mat& image, cv::Rect& area, cv::Size& size, const std::vector<rectangle>& trackers, const std::vector<rectangle>& faces, const std::vector<full_object_detection>& dets, const cv::Scalar color)
{
  ...
  //Using a Voting mechanism of 7 frames to determine gaze point
  int mode;
  ...
  //Show the gazepoint and send tag through Unix socket to Jellyfish Demo
...
// Training Data Collection interface for Random Forrests
  if(LeftEllipse.size() == 6 && RightEllipse.size() == 6 && screenPoint == 1){
    ...
    if(train == 0)
      trainForrest.open(train_path, std::ios_base::app);
    else
      trainForrest.open(test_path, std::ios_base::app);
      ...
      for (std::vector<cv::Point2f>::iterator i = LeftEllipse.begin(); i != LeftEllipse.end(); ++i)
      {
          ...
          first = false;
          trainForrest << (i->x - meanX)/stdevX <<","<<(i->y - meanY)/stdevY;
      }
    ...
    trainForrest<< (meanLeft.x - meanX)/stdevX <<"," << (meanLeft.y - meanY)/stdevY<<",";
    trainForrest<<leftDisplacement.x/stdevX<<","<<leftDisplacement.y/stdevY<<",";
    trainForrest<<rightDisplacement.x/stdevX<<","<<rightDisplacement.y/stdevY<<",";
    trainForrest<<(leftPupil.x - meanX)/ stdevX<<","<<(leftPupil.y - meanY)/stdevY<<",";
    trainForrest<<(lEllipse.center.x - meanX)/stdevX <<","<<(lEllipse.center.y - meanY)/ stdevY<<",";
    trainForrest<< (meanRight.x - meanX)/stdevX <<"," << (meanRight.y - meanY) /stdevY<<",";
    trainForrest<<(RightPupil.x - meanX)/stdevX<<","<<(RightPupil.y- meanY)/stdevY<<",";
    trainForrest<<(rEllipse.center.x - meanX)/stdevX<<","<<(rEllipse.center.y - meanY)/stdevY<<",";
    trainForrest<<meanX<<","<<meanY<<",";
    trainForrest<<stdevX<<","<<stdevY<<",";
  ...
  if(LeftEllipse.size() == 6 && RightEllipse.size() == 6 && screenPoint == 2){
    ...
    if(train == 0)
      trainForrest.open("/Users/Joey/Desktop/RForrest_train/std_forrest_train1.txt", std::ios_base::app);
    else
      trainForrest.open("/Users/Joey/Desktop/RForrest_train/std_forrest_test.txt", std::ios_base::app);
...
/********************** Unix Socket Initialization END ****************************/
  ...
  /***************** RANDOM TREES ****************/
            ...
            read_data_from_csv(test_path, testing_data, testing_classifications, NUMBER_OF_TESTING_SAMPLES))
    {
        ...
        rtree->save("models/normalized_Random_forrest_classifier.yml");
      ...
      rtree->load("models/normalized_Random_forrest_classifier.yml");
    }
        while(1)
        {
